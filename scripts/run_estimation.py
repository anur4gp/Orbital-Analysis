...
from orbital.estimation.measurements import RangeRangeRate
...
class ExperimentResults:
    """Monte Carlo histories grouped by prior case and filter name."""

    n_runs: int
    cases: dict[str, dict[str, MonteCarloResult]]
...
def cache_fingerprint() -> str:
    """Invalidate cached histories when source or numerical-library versions change."""
    digest = hashlib.sha256(f"{np.__version__}/{scipy.__version__}".encode())
    package_root = Path(orbital.__file__).parent
    for path in [Path(__file__), *sorted(package_root.rglob("*.py"))]:
        digest.update(path.read_bytes())
    return digest.hexdigest()


def load_results(data: dict[str, np.ndarray]) -> ExperimentResults:
    cases: dict[str, dict[str, MonteCarloResult]] = {}
    for case in CASES:
        cases[case] = {}
        for f in FILTERS:
            arrays = {
                field.name: data[f"{case}/{f.name}/{field.name}"]
                for field in fields(MonteCarloResult)
            }
            cases[case][f.name] = MonteCarloResult(**arrays)
    return ExperimentResults(int(data["n_runs"]), cases)


def run_or_load(n_runs: int, force: bool) -> ExperimentResults:
    if n_runs < 1:
    ...
    if CACHE.exists() and not force:
        with np.load(CACHE, allow_pickle=False) as archive:
            if ("fingerprint" in archive
                    and str(archive["fingerprint"]) == fingerprint
                    and int(archive["n_runs"]) == n_runs):
                return load_results(dict(archive))
        print("  Cached experiment differs from this run; recomputing.")
    out: dict[str, np.ndarray] = {
        "n_runs": np.array(n_runs),
        "fingerprint": np.array(fingerprint),
    }
    for case, (sigma_r, sigma_v) in CASES.items():
        t0 = time.perf_counter()
        results = monte_carlo(scenario(sigma_r, sigma_v), FILTERS, n_runs, seed=MC_SEED)
        print(f"  {case:<10} {n_runs} runs x {len(FILTERS)} filters in "
              f"{time.perf_counter() - t0:.0f} s")
        for name, result in results.items():
            for field in fields(MonteCarloResult):
                out[f"{case}/{name}/{field.name}"] = getattr(result, field.name)
    CACHE.parent.mkdir(parents=True, exist_ok=True)
    # Replace only a complete archive, preserving the previous run if writing fails.
    with NamedTemporaryFile(dir=CACHE.parent, suffix=".npz", delete=False) as temporary:
        temporary_path = Path(temporary.name)
    try:
        np.savez_compressed(temporary_path, **out)
...
def rtn_errors(
    t: np.ndarray, errors: np.ndarray, cov: np.ndarray, truth: np.ndarray,
...
def summary(data: ExperimentResults) -> None:
    n = data.n_runs
    ...
    for case, results in data.cases.items():
        for name, result in results.items():
            avg = cs.nees(result.errors, result.P).mean(axis=0)
            nis = result.nis[:, result.updated]
            nis_mean = float(nis.mean()) if nis.size else float("nan")
            pos = cs.rms_over_runs(result.errors[..., :3])[-1]
            vel = cs.rms_over_runs(result.errors[..., 3:])[-1]
            print(f"{case:<11}{name:<7}{avg[-1]:>9.1f}{avg.max():>10.1f}"
                  f"{cs.fraction_inside(avg[1:], (lo, hi)):>9.0%}{nis_mean:>10.2f}"
...
def diagnose_gap() -> None:
    ...
    if len(obs) < 2:
        print("\nGap diagnostic unavailable: fewer than two observations.")
        return
    gaps = np.diff([o.t_s for o in obs])
    gap_index = int(np.argmax(gaps))
    ekf = FILTERS[0]

    p, t = sc.p0, float(sc.t_s[0])
    for observation_index, o in enumerate(obs[: gap_index + 1]):
...
def shade_passes(ax: Axes, result: MonteCarloResult) -> None:
    updated_minutes = result.t_s[result.updated] / 60.0
    if not updated_minutes.size:
        return
    passes = np.split(updated_minutes, np.where(np.diff(updated_minutes) > 2)[0] + 1)
    for tracking_pass in passes:
        ax.axvspan(tracking_pass[0], tracking_pass[-1], color=BAND, lw=0, zorder=0)


def error_figure(data: ExperimentResults) -> None:
    truth = scenario(*CASES["TLE-grade"]).truth()
    minutes = T_GRID / 60.0
    fig, axes = plt.subplots(1, 2, figsize=(7.2, 2.7), sharey=True, constrained_layout=True)
    for tag, ax, f in zip("ab", axes, FILTERS, strict=True):
        result = data.cases["TLE-grade"][f.name]
        t = result.t_s / 60.0
        e_rtn, s_rtn = rtn_errors(result.t_s, result.errors[:1], result.P[:1], truth)
        colour, _ = STYLE[f.name]
        shade_passes(ax, result)
        ...
        ax.set_title(f"({tag}) {f.name}", loc="left")
...
def consistency_figure(data: ExperimentResults) -> None:
    ...
    ax = axes[2]
    shade_passes(ax, data.cases["TLE-grade"]["EKF"])
    for f in FILTERS:
        result = data.cases["TLE-grade"][f.name]
        colour, dash = STYLE[f.name]
        ax.semilogy(result.t_s / 60.0, cs.rms_over_runs(result.errors[..., :3]) * 1e3,
...
def figures(data: ExperimentResults) -> None:
    style()
    error_figure(data)
