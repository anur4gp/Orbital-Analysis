...
def temperature_schedule(m: int, t_min: float = 0.05, t_max: float = 5.0) -> np.ndarray:
    """Geometric ladder of replica temperatures, as the reference test uses."""
...
def maximin_criterion(design: np.ndarray, p: float = 15.0) -> float:
    """Morris-Mitchell phi_p -- lower is better, and approaches 1/min-distance
    as p grows. Mirrors phi() in maximin_PTLHD.cpp."""
    n = design.shape[0]
    diff = design[:, None, :] - design[None, :, :]
...
def generate_maxpro(
    ...
    """Run parallel-tempering MaxPro. Returns (design, measure, iterations)."""
...
def is_latin_hypercube(design: np.ndarray, tol: float = 1e-9) -> bool:
    """True if every column is a permutation of the (i - 0.5)/n levels."""
    n, k = design.shape
